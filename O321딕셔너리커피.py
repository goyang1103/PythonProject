...
while True:
    ...
    if menu == 1:
        for key in menu_dic:  # key의 개수 만큼 자동 순회
            print(key, ":", menu_dic[key])  # 키와 키에 해당하는 값을 출력
    elif menu == 2:
        read_name = input("조회 할 메뉴 입력 : ")
        ...
        if add_name not in menu_dic:
            ...
            desc = input("메뉴 설명 : ")
            menu_dic[add_name] = [cate, price, desc]  # 해당 키에 값을 대입, 값이 리스트 였으므로 리스트로 대입
        else:
            print("메뉴가 이미 존재 합니다.")
    elif menu == 4:
